...
const Supermercados = () => {
  ...
  useEffect(() => {
    if (data && data.supermercados) {
      setCardsData(data.supermercados);
    }
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={{padding:10,}}>
        {cardsData.map((_supermercados: ComerciosDataType, index: number) => (
          <CardComercios
            key={`supermercados_${index}`} comercios={{
              imagen: '',
              nombre: '',
              apellido: '',
              descripcion: '',
